...
import (
	...
	apisCluster "github.com/NitriKx/provider-cloudinaryprovisioning/apis/cluster"
	apisNamespaced "github.com/NitriKx/provider-cloudinaryprovisioning/apis/namespaced"
	"github.com/NitriKx/provider-cloudinaryprovisioning/config"
	"github.com/NitriKx/provider-cloudinaryprovisioning/internal/clients"
	controllerCluster "github.com/NitriKx/provider-cloudinaryprovisioning/internal/controller/cluster"
	controllerNamespaced "github.com/NitriKx/provider-cloudinaryprovisioning/internal/controller/namespaced"
	"github.com/NitriKx/provider-cloudinaryprovisioning/internal/features"
	"github.com/NitriKx/provider-cloudinaryprovisioning/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for CloudinaryProvisioning").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-cloudinaryprovisioning"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-cloudinaryprovisioning",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped CloudinaryProvisioning APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced CloudinaryProvisioning APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped CloudinaryProvisioning controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced CloudinaryProvisioning controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped CloudinaryProvisioning controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced CloudinaryProvisioning controllers")
	}
